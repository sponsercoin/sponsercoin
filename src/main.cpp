...
#if defined(NDEBUG)
#error "Sponsercoin cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < Params().LAST_POW_BLOCK())
        ...
        nSubsidy = 15 * COIN;
    else if (nHeight > 1050000 && nHeight <= 1900000)
        nSubsidy = 5 * COIN;
    else if (nHeight > 1900000 && nHeight <= 3000000)
        nSubsidy = 2.5 * COIN;
    else
        nSubsidy = 1.25 * COIN;
...
{
    RenameThread("sponsercoin-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // SPON: We use certain sporks during IBD, so check to see if they are
